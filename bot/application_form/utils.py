...
async def handle_contact(
    message: Message, state: FSMContext, session, fsm: State, answer: str
) -> bool | None:
    """
    ...
    try:
        ...
        # Логируем ошибку
        logger.error(
            f"Ошибка при выполнении команды {handle_contact.__name__} "
            f"( Обрабатывает получение номера телефона пользователя.): {e}"
        )
        await message.answer(
