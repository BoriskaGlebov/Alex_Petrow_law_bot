...
@application_form_router.callback_query(F.data.startswith('approve_'), ApplicationForm.approve_work)
...
@application_form_router.callback_query(F.data.startswith('owner_'), ApplicationForm.owner)
    ...
    try:
        ...
        async with ChatActionSender.typing(bot=bot, chat_id=call.message.chat.id):
            ...
            if can_contact_inf:
                ...
                # Убираем клавиатуру
                await call.message.answer(text='До свидания!', reply_markup=ReplyKeyboardRemove())
                # Очищаем состояние
                await state.clear()
    ...
    async with user_locks[user_id]:  # 🔒 Блокируем только для этого пользователя
        try:
            ...
            if not question_asked:
                ...
                await message.answer('Еще фото будете отправлять?', reply_markup=approve_keyboard("Да", "Нет"))

        except Exception as e:
            logger.error(f"Ошибка при обработке фото: {e}")
            await message.answer("Произошла ошибка при обработке вашего фото. Попробуйте снова.")
        finally:
...
@application_form_router.message(F.document.mime_type.startswith('image/'), ApplicationForm.photo)
@application_form_router.message(F.document.mime_type.startswith('image/webp'), ApplicationForm.photo)
...
@application_form_router.message(F.document | F.video | F.audio | F.voice, ApplicationForm.photo)
async def photo_mistakes(message: Message, state: FSMContext):
    ...
    try:
        ...
        mime_type = message.document.mime_type if hasattr(message, "document") else "Не документ"
        ...
        if hasattr(message, "document"):  # Если это документ
            ...
            await message.answer('Пожалуйста, отправьте фото, а не голосовое сообщение!')
            await state.set_state(ApplicationForm.photo)

    except Exception as e:
        # Логируем ошибку
        logger.error(f"Ошибка при обработке файла: {e}")
        await message.answer("Произошла ошибка при обработке вашего файла. Попробуйте снова.")


@application_form_router.callback_query(F.data.startswith('approve_'), ApplicationForm.photo)
...
async def bank_get_inf(message: Message, state: FSMContext):
    ...
    try:
        async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
            ...
            await message.answer("Укажите общую сумму, заблокированную в этом банке у данного лица.")
...
async def total_amount_get_inf(message: Message, state: FSMContext):
    ...
    try:
        ...
        match = re.search(r'[-+]?\d*\,?\d+', user_input.replace(' ', ''))  # Игнорируем пробелы
        async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
            if match:
                ...
                await message.answer("Пожалуйста, введите сумму с числами. Например: '1000' или '1000.50'.")
...
@application_form_router.callback_query(F.data.startswith('approve_'), ApplicationForm.new_bank)
@connection()
async def photo_callback(call: CallbackQuery, state: FSMContext, session) -> None:
    """
    ...
    try:
        ...
        async with ChatActionSender.typing(bot=bot, chat_id=call.message.chat.id):
            if new_bank_inf:
                ...
                total_amount: Optional[List[float]] = user_data.get('total_amount',
                                                                    None)  # Тип данных: Optional[List[float]]
                photos: List[str] = user_data.get('photos', [])  # Тип данных: List[str]

                # Создаем заявку в БД
                application_model = Application(user_id=user_info.id, status=status, owner=owner)
                ...
                response_message += f"Собственные счета - ДА\n\n" if application.owner else f"Собственные счета - Нет\n\n"
                ...
                await call.message.answer(response_message, reply_markup=approve_keyboard("ДА", "НЕТ, начать сначала."))
...
@application_form_router.callback_query(F.data.startswith('approve_'), ApplicationForm.approve_form)
@connection()
async def approve_form_callback(
    ...
    try:
        ...
        if approve_form_inf:
            ...
            response_message += f"Собственные счета - ДА\n\n" if last_appl.owner else f"Собственные счета - Нет\n\n"
