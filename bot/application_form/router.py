...
async def photo_mistakes(message: Message, state: FSMContext):
    ...
    try:
        ...
        print(message.document.mime_type if hasattr(message, "document") else "Не документ")
...
async def photo_callback(call: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        if new_bank_inf:
            ...
            await state.set_state(ApplicationForm.video)
            await bot.send_message(chat_id=call.message.chat.id,
                                   text="Просим вас попросить клиента записать видео, "
                                        "где он произносит на камеру с разворотом 2-3 страниц "
                                        "паспорта следующий текст: "
                                        "'Я, [ФИО], даю согласие на списание средств с заблокированных счетов'."
                                        " Приложите это видео и нажмите 'готово'.")
