...
async def help_cmd(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        command_list = [
            ...
            for cmd in (admin_commands if message.from_user.id in settings.ADMIN_IDS else user_commands)
        ...
        logger.bind(user=message.from_user.id).info(f'Команда /help выполнена пользователем {message.from_user.id}')
